...
"One authoritative tessellation per frame" survives, refined: one per hive, and the frame's picture is the tree's leaves — each level its own solve, hit-test, meters. While a field travels its domain is its cell's bounding box and the parent's border clips it: the one place a leaf is not its solved polygon, for the tenth of a second the parent is liquid. Exact at rest, which is where an eye can check.
