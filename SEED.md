...
**The border is a fact, not a wall.** A container nothing inside can bid against is enforced, and the owner will call that cheating — rightly. So the outer auction is open to pressure: a member asking for more than its rest share (hover, drag) bids for the extra in the page's auction too, as one more seed of its field placed where the member is. Inside the field that seed's cell is a seam, invisible. Where it reaches the border it carries straight on: a shard eating into the neighbour, whose straight edge is cut around it by a true bisector, every area exact, the neighbour's own lattice seeds recovering their loss as small axis-aligned steps elsewhere — pressure propagating through a rigid card, honestly. The field's outline, now bulged, is the interior's domain, and the interior re-tiles into it. At rest the extra claims are zero and every rectangle is exact; a locked grid inside lifts on hover instead of pressing, as the page does. The lemma is not violated, it is obeyed: the straight edge is straight exactly where nothing organic touches it.
