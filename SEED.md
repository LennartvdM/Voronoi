...
So while its crystal is between 0 and 1 a body is a HOLE in the auction's ground, and the hole's shape travels between the cell it would have (crystal 0) and its rectangle (crystal 1). The shape is carried as half-planes: the sides of the cell's convex hull and the rectangle's four sides, paired by direction, each pair one plane whose normal turns from the one to the other and whose offset is the support of the blend in that direction; convex by construction at every instant. The hull is the right thing to carry because a power cell is convex — what bends the cell it leaves behind is the ground's own cuts, a wall's corner or a stronger hole, and those cut the hole too, so hull less cuts is exactly the cell and the hole opens and closes on the cell itself. The cell it would have is read from a SIDE auction: the auction run once more with the body in and its own hole given back to the ground, everything else as it is, on its own warm weights and its own pocket — so the frame the hole closes and the first frame of the auction are one picture, and the weights it found are handed to the auction, which is then already converged. A body with nobody else in its pocket does not ask: its cell would be its rectangle and whatever ground lies about, and that ground it draws anyway. Holes never overlap a wall or each other — the more settled keeps its shape and the other loses exactly the overlap, as convex pieces — and the target follows the side auction on a short clock (80 ms), because the auction's frame-to-frame reshuffling is not the cell's motion. Voids stay as they were: their crystal is their vacators' clocks, not their own.

Three things had to be made exact underneath: the ground's pockets are found by shared edges, not by boxes, once the ground is wedges and not rectangles; a seed's pocket is the piece it stands in, not the box it is nearest; and a repeated vertex is no side — a zero plane made every cut emit its piece twice, which is where a page of overlaps came from. Measured on the cycle, cell against its own previous outline: a settling cell's excess move 6.7% to −2.4% median and 40% to 0.1% at the ninetieth percentile; a melting cell's 9.3% to 0.0% and 55% to 0.0%; switches moving more than 10% of the cell, 46 to 0. The cost, stated: mid-morph a cell has more sides than a bid (outline vertices at the ninetieth percentile 7 to 10, edges per neighbour 1.35 to 1.57), the per-frame reshaping mid-transition is livelier (frames with a body changing more than 12% of itself 23 to 66, none of them a switch), and the side auctions cost the stress case (30 elements, every one a field) a frame-time p95 of 32 ms against 23. Gap and overlap zero, exact rectangles at rest, nothing solved at rest.
