...
**What a cell may take on hover is bounded by what its neighbours can spare.** The ring a hovered cell pushes out is sized from its own rectangle: the depth a uniform growth would need to gain what the hover slider promises, and a third more. On a page of like-sized cells that is right, and it is what makes hover read as pressure rather than as a rectangle scaling up. On the hero layout it is a ring 113 px deep around a card 720 by 577, worth 311,827 px² — a quarter of the whole page — and the cards it leans on across the top are 116 px tall. They were left holding 528, 868 and 1,055 px² of rectangles worth 27,737, 41,606 and 55,474: two per cent, a bar eleven pixels tall where a number and a name should be. Then the page changes, the ring dies with the crystal that holds it, and a quarter of the page slides back across the top row in ten frames. The ring's arrival was already paced — it comes in over a journey, not over a tenth of a second, because every px² of it is cut out of somebody — but nothing bounded its DEPTH, and pacing the arrival of an annihilation does not stop it being one.

So the ring now stops at a share of the thinnest neighbour it actually reaches: a card 480 by 116 can give from its 116, and no ring may take more than a third of that. A cell whose neighbours have room grows exactly as it did — in the opening flock and in the bento the ring is 60,206 px² before and after, to the pixel — and a big cell beside small ones grows a little instead of eating them. That distinction is the whole rule, and it is invisible to a score: simply making every ring 40% shallower buys the same numbers on the hero layout and takes 61% off the rings that were never the problem.

Nothing about the auction moves. The pairs it solves, how far their walls slide, how much of that motion gets them anywhere, and how many pair-frames there are to measure are identical to the pixel and the sample. What changes is what the ring is allowed to take, and what the page does when it lets go: with the pointer on the hero card, the jumps outside the cold settle fall from 21 to 8 at 30 ms frames and from 11 to 7 at 60, the shock by a quarter to a third, the teleports from 119 to 99, and the over-reaching pieces from 1,073 to 1,023. With the pointer anywhere it is not crushing anything the file is bit-identical, field for field, at every clock — which is the shape a rule should have. The cost is on the other side of the same coin: with a pointer sweeping across the page the boost churn rises by a third, because a smaller ring means a smaller cell means a moving pointer leaves it sooner and picks up more cells on the way.
