...
The cell it would have is read from a SIDE auction: the auction run once more with the body in and its own hole given back to the ground, everything else as it is, on its own warm weights and its own pocket — so the frame the hole closes and the first frame of the auction are one picture, and the weights it found are handed to the auction, which is then already converged. A body with nobody else in its pocket does not ask: its cell would be its rectangle and whatever ground lies about, and that ground it draws anyway. The target follows the side auction on a short clock (80 ms), because the auction's frame-to-frame reshuffling is not the cell's motion. And a hole at crystal 0 does not close until its shape is the cell the auction would hand it: a bidder can arrive in its pocket in the very frame it lets go — a void opening beside it as the scene changes — and then the cell it gets is not the cell it aimed at; it keeps chasing on the short clock and closes when it has caught up, within 3%. Holes never overlap a wall or each other: the more settled keeps its shape and the other loses exactly the overlap, as convex pieces. Voids morph like everything else.
