...
const ANGLES = [
  { key: 'right-of-way', ask: 'GIVERS AND TAKERS. Some bodies have right of way and move; the others yield until the way is clear: hold, detour, or wait, decided by a rule (the larger claim, the shorter journey, the one whose path is crossed, percolation order), so movement is sequenced instead of simultaneous conflict. Design the rule, the yielding motion, and how a yielder rejoins.' },
  { key: 'anticipation', ask: 'ANTICIPATION. The auction is solved for where the seeds are going, not where they are: cells lead their seeds (solve on the carrot or on the seed extrapolated by its velocity over a short horizon), and a body reads its neighbours\' velocities (alignment) so it does not have to discover them by collision. Design what is predicted, over what horizon, and how the picture stays exact at rest.' },
  { key: 'choreography', ask: 'CHOREOGRAPHY. The paths themselves: an assignment that minimises crossings, journeys in waves, bezier control points bent so that paths do not cross, or departures in an order that leaves room. Design the assignment and the path shaping so that bodies rarely need to fight at all.' },
  { key: 'dynamics', ask: 'ONE MOTION LAW. Today a seed obeys an under-damped spring to its carrot, a repulsion from every liquid neighbour, a wobble, a wander, and the auction all at once, and they disagree. Design one consistent law for a travelling seed (critically damped, repulsion folded into the path or removed since cells cannot overlap anyway, wobble only at rest) so the seed never fights itself.' },
  { key: 'hysteresis', ask: 'CALM THE RECALCULATION. Everything the picture recomputes from scratch each frame that can flip between two answers: the holes\' cut order when crystals cross, pocket membership, the shadow target, the newcomer entry. Design hysteresis or continuity for each flip so that a body only changes its answer when the world has moved, not when it wobbled.' },
]
const designs = (await parallel(ANGLES.map(G => () => agent(`${COMMON}
You are on the design panel, angle "${G.key}". ${G.ask}
Read the brief, then the code the brief's code map names (steer, separate, updateCrystal, enterScene and its assignment, computeWalls, solveMain, solveShadow, placeSeeds). Here is what the diagnosis phase found, lens by lens:
${diagText}
