...
// when the change comes is on it too, where it is.
...
function cueReseat(h, v, r, content, T) {
  ...
  for (const b of content) {
    const p = b.path, j = b.journey; if (!p || !j || j.t0 !== h.t || Math.hypot(p.ex - p.sx, p.ey - p.sy) < 4) continue;
...
replace('''    for (const b of pending) this.retireBody(b, 0.8, tOf(b));
    ...
    if (name === 'cue' && cue) { cueAvoid(this, content); for (const [v, r] of cueSeat) cueReseat(this, v, r, content, tOf(v)); }   // CUE: the change looks ahead; a staying piece takes its new sites once the cells by it have gone by
...
Hive.prototype.placeSeeds = function() {   // CUE: whitespace makes way without giving ground
  ...
  for (const b of this.bodies) {   // the way it asks for comes in as it sets off and goes as it lands, by where its seed is, so nothing snaps back
    const p = b.path;
    if (b.isVoid || b.isSelf || b.leaving || !p || Math.hypot(p.ex - p.sx, p.ey - p.sy) < 4) continue;
