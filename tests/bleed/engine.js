...
const BL_YIELD_RATE = 800;    // px/s, 0 = no cap: how fast a tile may be moved aside to clear an overlap — a yield faster than a travel reads as a jump
  ...
  for (let it = 0; it < BL_PACK_ITERS; it++) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let j = i + 1; j < items.length; j++) {
        ...
        // a yield is a move like any other: no faster than a tile travels
        const lim = BL_YIELD_RATE ? BL_YIELD_RATE * h / BL_PACK_ITERS : Infinity;
        const mv = (d) => Math.min(Math.abs(d), lim) * Math.sign(d);
        if (axis === 'x') { A.x -= mv(sx * ox * share * r); B.x += mv(sx * ox * (1 - share) * r); }
        else { A.y -= mv(sy * oy * share * r); B.y += mv(sy * oy * (1 - share) * r); }
        moved = true;
      }
    }
  ...
  if (rigidPlanes.length) {
    ...
    for (const h of this.holes) {
      ...
      if (!pieces.length) {
        ...
        if (!big0 || bigA0 <= 1e-3) { h.hole = null; h.holeCore = null; continue; }
        pieces = [big0];
        undercut.push(unit(convexPlanes(big0)));
      }
      let big = pieces[0], bigA = 0;
      for (const p of pieces) { const a = Math.abs(ringArea(p)); if (a > bigA) { bigA = a; big = p; } }
