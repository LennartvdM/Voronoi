...
const BL_TRAVEL_CAP = 0.6;    // multiples of its own travel in this step: the furthest the packing may shove a TRAVELLER off its curve at once — what it cannot take falls on the other body, and what neither can take is what the cut takes (<0: no bound)
const BL_SEAT_CAP = 8;        // px per motion step: the same bound for a body that is not on its curve (<0: no bound)
const BL_PACK_ITERS = 4;      // packing sweeps per motion step: a chain needs one per link, and 93% of the overlaps that survived to be cut were in a chain
...
const BL_INK_MIN = 1;         // px²: a blended hole with less ink than this has no shape to show, and its body goes back to the auction
const BL_DEPART_ORDER = 1;    // a tile standing in a traveller's way departs no later than that traveller: nobody is cut if what was in the way has already left
const BL_DEPART_MEET = 0.5;   // share of the smaller area: how much of a waiter's footprint the sweep must cover before the waiter is held to be in the way
const BL_DEPART_PASSES = 8;   // passes of the departure cap: a chain of blockers all leave in time (a minimum over a fixed graph, so it settles)
...
const BL_REVIVE = 0.02;       // share of its fair area below which a seed is treated as a newcomer again: a crushed seed carries a weight the Newton cannot walk back from
const BL_ITERS = 40;          // Newton iterations the root may take in a frame: a page of thirty with the margin re-auctioning around it starts further from its answer than the reference's five ever have to (ten left the thirty-body change a tenth off, twenty a thousandth)
...
const BL_YIELD = 2;           // 0: every overlap on the packing's clock; 1: a waiter in a traveller's way yields the whole overlap in the frame it appears; 2: every pair that can move at all parts in that frame, and only a pair that cannot is left to be cut
...
Hive.prototype.blPack = function(h) {
  ...
  for (const b of this.bodies) {
    ...
    // HOW FAR THIS BODY MAY BE SHOVED IN THIS STEP. A traveller's painted
    // position is its curve plus the packing's displacement: taking a whole
    // overlap out of it at once is a jump the strobe sees. It is bounded by
    // what the body itself travels in the step — a traveller standing still
    // on its curve is not shoved at all, one at speed may be moved by about
    // what it was going to move anyway — so the packing never outruns the
    // motion it is packing.
    const bud = moving
      ? (BL_TRAVEL_CAP >= 0 ? BL_TRAVEL_CAP * Math.hypot(b.vx || 0, b.vy || 0) * h : Infinity)
      : (BL_SEAT_CAP >= 0 ? BL_SEAT_CAP : Infinity);
    items.push({ b, hw: fp.hw, hh: fp.hh, x: at.x, y: at.y, comp, pk, wx, wy, x0: at.x, y0: at.y, lx, ly, tr: moving, bud });
  }
  const live = new Set();
  ...
  for (let it = 0; it < BL_PACK_ITERS; it++) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let j = i + 1; j < items.length; j++) {
        ...
        if (BL_YIELD >= 2) { r = 1; if (A.tr !== B.tr && (A.tr ? B : A).comp > 0) share = A.tr ? 0 : 1; }
        else if (BL_YIELD && A.tr !== B.tr && (A.tr ? B : A).comp >= 1) { share = A.tr ? 0 : 1; r = 1; }
        ...
        // WHO ACTUALLY TAKES IT. A moves against the axis, B along it; each
        // only as far as it has left this step. What one cannot take falls
        // on the other, and what neither can take is exactly the overlap the
        // cut will take.
        const o = axis === 'x' ? ox : oy, sg = axis === 'x' ? sx : sy;
        const cur = q => axis === 'x' ? q.x - q.x0 : q.y - q.y0;
        const roomA = A.comp > 0 ? Math.max(0, A.bud + sg * cur(A)) : 0;
        const roomB = B.comp > 0 ? Math.max(0, B.bud - sg * cur(B)) : 0;
        const want = o * r;
        let dA = want * share, dB = want - dA;
        if (dA > roomA) { dB += dA - roomA; dA = roomA; }
        if (dB > roomB) { dA = Math.min(roomA, dA + (dB - roomB)); dB = roomB; }
        if (dA + dB <= 1e-9) continue;
        if (axis === 'x') { A.x -= sg * dA; B.x += sg * dB; }
        else { A.y -= sg * dA; B.y += sg * dB; }
        moved = true;
      }
    }
...
Hive.prototype.computeWalls = function() {
  ...
  for (const b of this.bodies) {
    ...
    if (b.isVoid && !b.blIsReserve && b.rect && !b.leaving) {
      ...
      // The reference's own rule for a wall is the test — crystal home, or
      // locked on a cell that is already its rectangle — because whatever
      // it calls a wall is cut out of the ground and cuts whatever it lies
      // across, and a void at crystal 0.9998 was doing exactly that
      const r = b.rect, rect = [r[0] * PW, r[1] * PH, r[2] * PW, r[3] * PH];
      if (!(b.crystal >= 1 || ((b.pin || 0) > 0 && this.cellIsRect(b, r)))) continue;
  ...
  // and a traveller at the end of its curve does not tie with a tile that
  // has landed and pinned: a tie was broken by body order, and half the time
  // it gave the cut to the one body that cannot move out of the way
  const rank = t => t.isVoid ? -1 : (t.landed && t.onSlot ? 3 : (t.fp.travelling ? 2 + 0.99 * (t.b.progress || 0) : 1 - this.blCompliance(t.b)));
  ...
  if (rigidPlanes.length) {
    ...
    for (const h of this.holes) {
      ...
      pieces = pieces.filter(p => p.length >= 3 && Math.abs(ringArea(p)) > 1e-3);
      // A BODY IS NEVER CUT AWAY TO NOTHING. A melting cell a tile has
      // arrived on top of would leave the page for the frames it takes the
      // tile to settle — a body blinking out and back is the worst thing
      // the picture can do. It keeps the largest piece it had, and the
      // tile above it gives that piece up instead (the last resort, taken
      // out of the rectangle this time, and counted as such).
      if (!pieces.length) {
        let big0 = null, bigA0 = 0;
        for (const p of h.hole.pieces) { const a = Math.abs(ringArea(p)); if (a > bigA0) { bigA0 = a; big0 = p; } }
        if (big0 && bigA0 >= BL_INK_MIN) { pieces = [big0]; undercut.push(unit(convexPlanes(big0))); }
      }
      // LESS THAN A PIXEL OF INK IS NO INK. The reference sends a body whose
      // hole is gone back to the auction, where it is a point site again; a
      // hole cut down to a sliver is the same thing with a rounding error
      // attached — it has no shape to show, it bids for nothing, and the body
      // paints two tenths of a pixel and reads as vanished.
      let ink = 0;
      for (const p of pieces) ink += Math.abs(ringArea(p));
      if (ink < BL_INK_MIN) { h.hole = null; h.holeCore = null; continue; }
      let big = pieces[0], bigA = 0;
      for (const p of pieces) { const a = Math.abs(ringArea(p)); if (a > bigA) { bigA = a; big = p; } }
    ...
    this.holes = kept;
    // and the tiles give up what they took from it
    for (const planes of undercut) for (const t of list) {
      const b = t.b;
      if (!b.hole || !b.blRigid) continue;
      let pieces = b.hole.pieces;
      const next = [];
      for (const p of pieces) next.push(...subtractPlanes(p, planes));
      pieces = next.filter(p => p.length >= 3 && Math.abs(ringArea(p)) > 1e-3);
      if (!pieces.length) continue;                       // it would leave nothing: the blend keeps its piece and this one keeps its rectangle
      let big = pieces[0], bigA = 0;
      for (const p of pieces) { const a = Math.abs(ringArea(p)); if (a > bigA) { bigA = a; big = p; } }
      b.hole = { pieces, planes: pieces.map(p => unit(convexPlanes(p))), rect: b.hole.rect, pts: big, raw: b.hole.raw };
    }
  }
...
Hive.prototype.solveMain = function() {
  ...
  r.blExtra = [];
  // A CRUSHED SEED COMES BACK AS A NEWCOMER. The reference revives a seed
  // whose cell came out EMPTY, by bisecting its weight to the area it is
  // painted at; a seed left owning a fiftieth of what it claims is in the
  // same place but not caught by that test, and the weight it carries —
  // a hundred thousand below its neighbours' — is one the Newton cannot
  // walk back: the line search refuses every step and the frame is left
  // as it is (in the thirty-to-four stress it was three frames off by
  // half their area). Such a seed is handed back to the entry rule, which
  // brings it in at the area it is drawn at this instant.
  if (BL_REVIVE) {
    let tot = 0, area = 0;
    for (const b of this.bodies) for (const q of b.subs) if (q.claim >= ACTIVE_MIN && !b.hole && !b.wall) tot += q.claim;
    const [x0, y0, x1, y1] = this.blBox();
    area = (x1 - x0) * (y1 - y0);
    if (tot > 1e-9) for (const b of this.bodies) for (const q of b.subs) {
      if (!q.live || b.hole || b.wall || q.claim < ACTIVE_MIN) continue;
      if ((q.blPaint || 0) >= BL_REVIVE * q.claim * area / tot) continue;
      // and it comes back at the area it CLAIMS, not the speck it was left
      // holding: entering at a fiftieth of a cell is entering crushed
      q.live = false; q.blPaint = 0; q.shadowLive = false;
    }
  }
...
Hive.prototype.enterScene = function(name, origin) {
  ...
  try { const out = blOldEnter.call(this, name, origin); if (BL_DEPART_ORDER) this.blDepartOrder(); if (BL_ROUTE) this.blRoute(); return out; }
  finally { if (r) this.bodies.push(r); }
};

// DEPART IN ORDER. Nobody is cut if the tile in the way has already left.
// At a scene change every tile takes a departure delay from the reference's
// passageTimes — the change spreading through the page — and a tile whose
// delay is later than that of a traveller that has to cross it is a tile
// the traveller must fit past: the packing parts them by what it can in the
// frames it has, and every frame of that crossing that is left over is a
// notched frame. So the change carries a DEPENDENCY: where a traveller T
// will sweep across a waiter S, and S is going somewhere outside that
// sweep, S leaves no later than T. The rule is a minimum over a fixed graph
// — the geometry is read once, at the change, before anything moves — so it
// settles in a few passes and a chain of blockers all leave in time. It
// never delays anybody: a delay only ever comes down.
Hive.prototype.blDepartOrder = function() {
  if (this.depth !== 0) return 0;
  const PW = this.PW, PH = this.PH, t0 = this.t;
  const mv = [];
  for (const b of this.bodies) {
    const j = b.journey;
    if (!j || !j.stamp || j.t0 !== t0 || !b.rect) continue;   // this change's travellers, in the page's own order
    const fp = this.blFootprint(b);
    if (!fp) continue;
    const at = this.blPlace(b, fp), r = b.rect;
    const from = [at.x - at.hw, at.y - at.hh, at.x + at.hw, at.y + at.hh];
    const dest = [r[0] * PW, r[1] * PH, r[2] * PW, r[3] * PH];
    mv.push({ b, j, from, dest,
      swept: [Math.min(from[0], dest[0]), Math.min(from[1], dest[1]), Math.max(from[2], dest[2]), Math.max(from[3], dest[3])],
      dx: (dest[0] + dest[2]) / 2, dy: (dest[1] + dest[3]) / 2 });
  }
  if (mv.length < 2) return 0;
  const area = q => Math.max(0, q[2] - q[0]) * Math.max(0, q[3] - q[1]);
  const meet = (a, q) => Math.max(0, Math.min(a[2], q[2]) - Math.max(a[0], q[0])) * Math.max(0, Math.min(a[3], q[3]) - Math.max(a[1], q[1]));
  const caps = [];
  for (const T of mv) {
    const aT = area(T.swept);
    if (!(aT > 0)) continue;
    for (const S of mv) {
      if (S === T) continue;
      const aS = area(S.from);
      if (meet(S.from, T.swept) <= BL_DEPART_MEET * Math.min(aS, aT)) continue;
      // and S is going somewhere else: a waiter whose own slot is inside the
      // sweep is in the way wherever it stands, and leaving early cannot help
      if (S.dx > T.swept[0] && S.dx < T.swept[2] && S.dy > T.swept[1] && S.dy < T.swept[3]) continue;
      caps.push([S, T]);
    }
  }
  if (!caps.length) return 0;
  let changed = 0;
  for (let pass = 0; pass < BL_DEPART_PASSES; pass++) {
    let any = false;
    for (const [S, T] of caps) if (S.j.delay > T.j.delay + 1e-9) { S.j.delay = T.j.delay; any = true; changed++; }
    if (!any) break;
  }
  // a tile's field re-rosters on the tile's own clock (the reference sets
  // fieldAt from the same delay): a tile that leaves earlier takes its
  // field's roster with it
  if (changed) for (const m of mv) { m.b.T = m.j.delay; if (m.b.fieldAt >= 0) m.b.fieldAt = t0 + m.j.delay; }
  this.blDeparted = changed;
  return changed;
};
...
Hive.prototype.hitTestAny = function(x, y) {
  ...
  if (!b || b.blIsReserve) return null;
  // nor a body the page is not holding right now: the walls of the frame
  // outlive a body the roster has reaped, and the graph that spreads a
  // change looks every hit up in its own map of the page's bodies
