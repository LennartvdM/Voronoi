...
function loadEngine(file,{width=1900,height=810,fields=.55,count=12,profile=false,record=false,transform=x=>x}={}) {
 ...
 const instrument=profile?functions.map(n=>`${n}=wrap('${n}',${n});`).join('\n')+methods.map(n=>`if(Hive.prototype.${n}) Hive.prototype.${n}=wrap('Hive.${n}',Hive.prototype.${n});`).join('\n'):'';
 const end=js.lastIndexOf('})();');
 js=js.slice(0,end)+instrument+`\nreturn {root,config,init,resize,computeDiagram,solveWeights,rasterCheck,advance(ms){tick(ms);return picture;},pointer(x,y){mouseX=x;mouseY=y;},scene(n){config.scene=n;root.enterScene(n);},time(){return simTime;},end(){return changeEnds(root);},inner(n){config.inner=n;root.walk(h=>{if(h.depth>0)h.enterScene(n);});}};\n`+js.slice(end);
 const noop=()=>{}, commands=[];
 const ctx={};
 for(const name of ['arc','arcTo','beginPath','clearRect','clip','closePath','fillRect','fillText','lineTo','moveTo','restore','save','setTransform','stroke'])ctx[name]=record?(...a)=>commands.push([name,...a]):noop;
