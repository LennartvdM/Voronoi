// Tell II adds a second scroll-tell: a small cluster, a blurb in a box in
// the open, and slides that send one, two or three cells to the stage around
// the box and back into the cluster when their turn is done. Without the Tell button, the real tick must produce exactly
...
const inside=(r,g)=>r[0]>=g[0]-1e-6&&r[1]>=g[1]-1e-6&&r[2]<=g[2]+1e-6&&r[3]<=g[3]+1e-6;
const overlap=(a,b)=>Math.min(a[2],b[2])-Math.max(a[0],b[0])>1e-6&&Math.min(a[3],b[3])-Math.max(a[1],b[1])>1e-6;
const clusterOf=e=>{const C=e.root.COLS,R=e.root.ROWS;return e.root.W<e.root.H?[0.34*C,0.70*R,C,R]:[0.83*C,0.22*R,C,0.78*R];};
// THE CAST TAKES TURNS: slide k's cast is the next cells in id order, round
// and round, as many as its stage has places
function expectedCast(e){const cells=roots(e).slice().sort((a,b)=>a.id-b.id),out=[];let next=0;for(const sd of e.slides2()){const cast=[];for(let i=0;i<Math.min(sd.stage.length,cells.length-1);i++)cast.push(cells[next++%cells.length]);out.push(cast);}return out;}
// A SLIDE, SETTLED: every cell of the cast stands exactly on its place, a
// rectangle, the page's image (no label, no hover); every other cell is in
// the cluster; the whitespace, the page less the stage and the cluster, is
// exactly its rectangles; every cell one site, none a field, none fractured,
// the count kept; the blurb titled with the cast's names inside the box and
// nowhere else, its lines set, the rule drawn.
function settled(st,k,label){
 const e=st.e,T=e.tell2();assert(T&&T.k===k,label+': the story is on slide '+(T&&T.k)+', not '+k);
 const spec=e.tell2Page(k,N),cast=T.heroes[k];
 assert.equal(T.on.length,cast.length,label+': '+T.on.length+' on stage, the cast is '+cast.length);
 assert.equal(cast.length,Math.min(e.slides2()[k].stage.length,N-1),label+': the cast is not the size of the stage');
 cast.forEach((b,i)=>{assert(T.on[i]===b,label+': '+b.name+' is not on stage');assert(!b.leaving,label+': '+b.name+' is leaving');
 ...
 for(let i=0;i<cast.length;i++)for(let j=i+1;j<cast.length;j++)assert(!overlap(cast[i].rect,cast[j].rect),label+': two of the cast overlap');
 assert(e.focus()===cast[0],label+': the first of the cast is not the page\'s image');
 const cl=clusterOf(e);
 for(const q of roots(e))if(!cast.includes(q)){assert(inside(q.rect,cl),label+': '+q.name+' is not in the cluster: '+rectKey(q.rect));assert(!q.journey||q.progress>=1,label+': '+q.name+' is travelling');}
 ...
 // the cast are rectangles: every point of a cast cell's outline lies on its rectangle's boundary, to a px
 {const PW=e.root.PW,PH=e.root.PH;for(const b of cast){const l=rootsOf(st.pic).find(l=>l.body===b);assert(l,label+': '+b.name+' has no cell');const r=[b.rect[0]*PW,b.rect[1]*PH,b.rect[2]*PW,b.rect[3]*PH];
  for(const q of l.loops[0])assert(q[0]>r[0]-1&&q[0]<r[2]+1&&q[1]>r[1]-1&&q[1]<r[3]+1&&(Math.abs(q[0]-r[0])<1||Math.abs(q[0]-r[2])<1||Math.abs(q[1]-r[1])<1||Math.abs(q[1]-r[3])<1),label+': '+b.name+' is not a rectangle at '+q.map(v=>v.toFixed(1)));}}
 ...
 // the whitespace is what the cells leave: no void rectangle overlaps a cell's, and together they cover the page
 ...
 const title=cast.map(b=>b.name).join(' · '),texts=e.commands.filter(c=>c[0]==='fillText'&&c[1]===title);
 ...
 return{k,cast:cast.map(b=>b.name),places:e.slides2()[k].stage.slice(0,cast.length),voids:voids.length,rest:{rectangle:sh.rectangle,voronoi:sh.voronoi,cut:sh.cut,shortEdged:sh.shortEdged}};
}
...
{ // the story begins: the cast chosen in turns, slide 0's on the stage, the rest in the cluster
 ...
 const want=expectedCast(e);e.tell2().heroes.forEach((c,k)=>assert(c.length===want[k].length&&c.every((b,i)=>b===want[k][i]),'slide '+k+'\'s cast is '+c.map(b=>b.name)+', not '+want[k].map(b=>b.name)));
 for(let k=1;k<want.length;k++)assert(!want[k].some(b=>want[k-1].includes(b)),'a cell is on stage two slides running at slide '+k);
 assert(want.some(c=>c.length===3)&&want.some(c=>c.length===2)&&want.some(c=>c.length===1),'the slides do not show one, two and three cells');
 report.text=through(st,'start');
 report.start=settled(st,0,'slide 0');report.slides.push(report.start);
 // a wheel notch: the next slide's cast comes out as the last goes back into the cluster; the text stays; no fracture
 const prev=e.tell2().on.slice();e.tellPush&&0;e.tell2Push(100);let changed=-1,minAlpha=1,frac=0;
 for(let f=0;f<420;f++){st.step(1);const T=e.tell2();minAlpha=Math.min(minAlpha,T.alpha);frac+=shapes(st).fractured;if(changed<0&&T.k===1)changed=f;}
 assert(changed>=0,'the notch did not turn the slide');assert.equal(frac,0,'a fractured cell in the slide change');
 assert(minAlpha>0.97,'the text was hidden by the slide change: alpha '+minAlpha.toFixed(2));
 report.push={changedAt:changed,...settled(st,1,'slide 1')};report.slides.push(report.push);
 for(const b of prev)assert(inside(b.rect,clusterOf(e)),b.name+' did not go back into the cluster');
 // a drag and a flick: the story runs on past slides and settles on one, exact
 ...
 frac=0;for(let f=0;f<480;f++){st.step(1);frac+=shapes(st).fractured;}assert.equal(frac,0,'a fractured cell in the flick');
 ...
 // every slide on: each cast on its places, the last back in the cluster
 for(let k=kd+1;k<e.slides2().length;k++){const last=e.tell2().on.slice();e.tell2Go(k);st.step(420);report.slides.push(settled(st,k,'slide '+k));for(const b of last)if(!e.tell2().on.includes(b))assert(inside(b.rect,clusterOf(e)),b.name+' did not go back into the cluster at slide '+k);}
 ...
 // back to the start: the same cast, the same places
 e.tell2Go(0);st.step(420);assert.equal(e.tell2().k,0,'the story did not go back to slide 0');
 ...
 assert(!e.root.bodies.some(q=>q.rect&&q.rect.reelLive),'the whitespace kept its handed sites at home');
...
{ // a notch is a slide, whatever its size: one on, a tiny one on, one back; three quick ones spin the story on by less than three
 const st=fresh(desk),e=st.e;e.tell2Start();st.step(420);
 ...
 e.tell2Push(100);st.step(10);e.tell2Push(100);st.step(10);e.tell2Push(100);st.step(420);const spun=e.tell2().k-2;assert(spun>=1&&spun<=2,'three quick notches turned '+spun+' slides');
...
{ // the snap: a short move eases back to its slide, a long one on to the next
 const st=fresh(desk),e=st.e;e.tell2Start();st.step(420);
 e.tell2Move(0.3*e.root.H);st.step(180);assert.equal(e.tell2().k,0,'a short move turned the slide');assert(e.tell2().y<0.5,'a short move did not ease back: y '+e.tell2().y.toFixed(1));
 e.tell2Move(0.6*e.root.H);st.step(420);assert.equal(e.tell2().k,1,'a long move did not turn the slide');assert(Math.abs(e.tell2().y-e.root.H)<0.5,'a long move did not settle on the slide: y '+e.tell2().y.toFixed(1));
...
{ // a notch during the entry: the change that wins is the slide's, and it ends exact
 ...
 let frac=0,k=-1;for(let f=0;f<480;f++){st.step(1);frac+=shapes(st).fractured;if(k<0&&e.tell2().k!==0)k=f;}
...
{ // a phone: the box a band across the top, the stage a row below it, the cluster at the foot; the story turns the same way
 const st=fresh(phone),e=st.e;e.tell2Start();
 const tx=through(st,'phone');const r0=settled(st,0,'phone slide 0');
 const R=e.root.ROWS,C=e.root.COLS;for(const b of e.tell2().on)assert(b.rect[1]>=0.36*R-1e-6&&b.rect[3]<=0.56*R+1e-6,'phone: the stage is not the row below the band');
 e.tell2Push(100);st.step(420);const r1=settled(st,1,'phone slide 1');
 assert(r1.cast.length===2,'phone: slide 1 does not show two cells');
 e.tell2Go(3);st.step(420);const r3=settled(st,3,'phone slide 3');assert(r3.cast.length===3,'phone: slide 3 does not show three cells');
 report.phone={text:tx,slides:[r0,r1,r3]};
}
const result={scope:'Real full tick; native Canvas and browser DOM stubbed. Without the Tell II button, state and every drawing command must exactly match Tell, a click, a scroll, a Tell story and home included; with the story, every slide\'s cast (one, two or three cells, taking turns) stands exactly on its places around the box and the rest in the cluster, the whitespace exactly the rectangles the cells leave, every cell one site and never fractured, the last cast back in the cluster, the text set once the entry has ended and the rule a beat later and kept through every slide, a wheel notch of any size turning one slide, a drag and a flick carrying the story on, a slow story snapping to its slide, the story stopping at its ends, and home returning the count.',hashes:files.map(f=>({file:path.basename(f),sha256:hash(fs.readFileSync(f))})),matrix,totalFrames:total,drawingCommands:commandsTotal,story:report};
