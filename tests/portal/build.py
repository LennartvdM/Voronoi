...
let portalFlip = false;                             // the page mirrored left-right, so it opens on the side clicked
...
function portalPage(T0, C, R, n, W, H) {
  let T = W < H ? PORTAL_PORTRAIT[T0.text ? 'text' : 'caption'] : Object.assign({}, T0, T0.fit ? T0.fit(W, H) : {});
  if (portalFlip && W >= H) {
    // mirrored left-right: the image's slot on the side the click came from
    const M = f => [1 - f[2], f[1], 1 - f[0], f[3]], side = { left: 'right', right: 'left' };
    T = { hero: M(T.hero), text: T.text && M(T.text), margins: (T.margins || []).map(M), galleries: T.galleries.map(M), rules: T.rules.map(e => side[e] || e) };
  }
...
function portalDecorate(content, voids, W, H, C, R) {
  ...
  const vs = voids.map((v, k) => {
    ...
    q.mfKey = JSON.stringify(q.mfSites); q.mfRest = true; if (v.portalText) { q.portalText = true; q.portalRules = v.portalRules; }
    return q;
  });
...
function portalScene(h, name, n) {
  const key = [name, h.COLS, h.ROWS, n, h.W, h.H, portalFlip ? 'm' : ''].join('|');
...
''')
replace('''        const f = den > 0 ? Math.max(tb.f, Math.min(1, num / den)) : 1;''', '''        const f0 = den > 0 ? Math.min(1, num / den) : 1;
        const f = Math.max(tb.f, tb.wait ? Math.min(f0, b.progress) : f0);   // PORTAL: a page's whitespace opens no faster than its own clock, which waits for the cells crossing it''')
replace('''    for (const b of content) {
    ...
    // PORTAL: a page's long journeys take the time they need, so no cell
    // rides the speed cap: the peak of an eased trip is one and a half
    // times its average
    if (name in PORTAL_TEMPLATES) for (const b of content) { const j = b.journey, p = b.path; if (j && p && j.t0 === this.t) j.dur = Math.max(j.dur, Math.hypot(p.ex - p.sx, p.ey - p.sy) / 600); }
    // PORTAL: the image grows no faster than the page clears for it: its
    // journey, and so its claim, ends when the last card's does
    if (name in PORTAL_TEMPLATES && portalFocus && portalFocus.journey && portalFocus.journey.t0 === this.t) {
      let last = 0; for (const b of content) if (b !== portalFocus && b.journey) last = Math.max(last, b.journey.delay + (b.journey.hold || 0) + b.journey.dur);
      const j = portalFocus.journey; j.dur = Math.max(j.dur, last - j.delay - (j.hold || 0));
    }''')
replace('''    for (const v of newVoids) {
      const r = rectOfVoid.get(v), rows = []; let W = 0;
      for (const b of this.bodies) {
        if (b === v || b.isSelf || rectOfVoid.has(b) || stays.has(b)) continue;
        if (this.inRect(r, b.x, b.y)) { rows.push({ body: b, journey: b.journey, w: b.claim }); W += b.claim; }
      }
      this.seatBody(v, r, 0, { t0: this.t, delay: 0, dur: 0.9, c0: 0 });
      v.table = { mode: 'open', rows, wRem: Math.max(0, rectArea(r) - W), f: 0 };
    }''', '''    for (const v of newVoids) {
      const r = rectOfVoid.get(v), rows = []; let W = 0, wait = 0;
      for (const b of this.bodies) {
        if (b === v || b.isSelf || rectOfVoid.has(b) || stays.has(b)) continue;
        if (this.inRect(r, b.x, b.y)) { rows.push({ body: b, journey: b.journey, w: b.claim }); W += b.claim; continue; }
        // PORTAL: a page's whitespace also waits for the cells whose journeys
        // pass through it, so nothing crosses it as a sliver and its text is
        // not set under a traveller: they are on its ledger, and it does not
        // open on its own clock until the last of them is through
        if (!(name in PORTAL_TEMPLATES) || !b.path || !b.journey || b.journey.t0 !== this.t) continue;
        const p = b.path; let last = -1;
        for (let k = 1; k < 20; k++) { const e = k / 20, m = 1 - e, x = m * m * p.sx + 2 * m * e * p.cx + e * e * p.ex, y = m * m * p.sy + 2 * m * e * p.cy + e * e * p.ey; if (this.inRect(r, x, y)) last = e; }
        if (last < 0) continue;
        rows.push({ body: b, journey: b.journey, w: b.claim }); W += b.claim;
        const u = last < 0.5 ? Math.cbrt(last / 4) : 1 - Math.cbrt(2 * (1 - last)) / 2;   // easeInOutCubic, inverted
        wait = Math.max(wait, b.journey.delay + (b.journey.hold || 0) + b.journey.dur * Math.min(1, u + 0.05));
      }
      this.seatBody(v, r, 0, { t0: this.t, delay: wait, dur: 0.9, c0: 0 });
      v.table = { mode: 'open', rows, wRem: Math.max(0, rectArea(r) - W), f: 0 };
      if (wait > 0) v.table.wait = wait;   // a page's whitespace only: a scene's table is as it was
    }''')
