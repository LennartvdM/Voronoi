...
const files={caption:path.resolve(__dirname,'../../caption.html'),plaque:path.resolve(__dirname,'../../plaque.html')};
const cases=[
...
for(const cfg of cases){
 for(let w=0;w<2;w++)for(const name of ['caption','plaque'])workload(loadEngine(files[name],cfg),false,cfg.moving);
 const samples={caption:[],plaque:[]};
 for(let rep=0;rep<5;rep++)for(const name of rep%2?['plaque','caption']:['caption','plaque'])samples[name].push(workload(loadEngine(files[name],cfg),true,cfg.moving));
 ...
 const row={...cfg,framesPerRun:1600,repeats:5,medians,reductionPct:100*(1-medians.plaque.totalMs/medians.caption.totalMs),samples};
