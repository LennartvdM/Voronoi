...
function loadEngine(file,{width=1900,height=810,fields=.55,count=12,profile=false,record=false,transform=x=>x}={}) {
 ...
 js=js.slice(0,end)+instrument+`\nreturn {root,config,init,resize,computeDiagram,solveWeights,rasterCheck,advance(ms){tick(ms);return picture;},pointer(x,y){mouseX=x;mouseY=y;},scene(n){config.scene=n;root.enterScene(n);},time(){return simTime;},changeLeft(){return guestLeft;},end(){return changeEnds(root);},inner(n){config.inner=n;root.walk(h=>{if(h.depth>0)h.enterScene(n);});}};\n`+js.slice(end);
 const noop=()=>{}, commands=[];
 const ctx={};
 ...
 // Plaque measures text; the stub answers with a deterministic width from the font's px size.
