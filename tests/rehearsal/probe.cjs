...
function loadEngine(file,{width=1900,height=810,fields=.55,count=12,profile=false,record=false,transform=x=>x}={}) {
 ...
 e.think({ms:Infinity,frames:Infinity});   // a probe thinks at once, within the frame, so what it counts does not hang on the machine's speed
