...
function loadEngine(file, width = 1900, height = 810, fields = 0, count = 12, transform = x => x) {
  const html = fs.readFileSync(file, 'utf8');
  let js = transform(html.match(/<script>([\s\S]*?)<\/script>/)[1]);
  const routing = js.slice(js.indexOf('  root.walk(h => { h.hoveredId = -1; });'), js.indexOf("  canvas.style.cursor = 'default';"));
  ...
  js = js.slice(0, end) + `return {root, config, init, advance(dt) {
    ...
    ${routing}
    return picture;
  }, pointer(x,y) {mouseX=x; mouseY=y;}, end() {return changeEnds(root);}, time() {return simTime;}, scene(name) {config.scene=name; root.enterScene(name);}};\n` + js.slice(end);
  const noop = () => {};
