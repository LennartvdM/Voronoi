...
function loadEngine(file,{width=1900,height=810,fields=.55,count=12,profile=false,record=false,transform=x=>x}={}) {
 ...
 e.budget(8);   // a probe thinks a counted amount each frame (a desk's measured budget), so what it counts does not hang on the machine's speed
