...
function tesseraSolve(seeds, owner, offset, targets, w0, bounds, maxIter = 16, maxEvals = 32) {
  ...
  // REVIVAL. A group's weight comes from the last frame it was solved in,
  // and a frame can leave a group with no cell at all (whitespace coming
  // back on its page lattice at the weight it had when it was most of the
  // page crushes the tiles beside it; a body born at the minimum claim next
  // to another). Newton has no derivative for a cell that is not there, so
  // a dead group first has its own weight alone raised by bracketing and
  // bisection until it holds about its target (Astra I's own revival pass,
  // at a fifth of the evaluations). Groups merely far from their targets are
  // left to Newton: a pass that pushed each of those to its target in turn
  // crushed its neighbours in turn and cost more than it saved.
  const repair = (i) => {
    const want = target[i];
    let a = current.areas[i];
    const up = a < want;
    let step = area / n / 4, prev = w[i];
    for (let k = 0; k < 12; k++) {
      prev = w[i]; w[i] += up ? step : -step; current = evaluate(w); a = current.areas[i];
      if (up ? a >= want : a <= want) break;
      step *= 2;
    }
    if (!(up ? a >= want : a <= want)) return;
    let lo = up ? prev : w[i], hi = up ? w[i] : prev;
    for (let k = 0; k < 8; k++) {
      w[i] = (lo + hi) / 2; current = evaluate(w);
      if (current.areas[i] < want) lo = w[i]; else hi = w[i];
      if (Math.abs(current.areas[i] - want) < 0.1 * want) break;
    }
  };
  // (twice over if need be: two newborn bodies a few px apart can each
  // bury the other, and one pass leaves the second one dead)
  for (let pass = 0; pass < 3; pass++) {
    let any = false;
    for (let i = 0; i < n; i++) if (current.areas[i] <= 1e-3 * target[i]) { repair(i); any = true; }
    if (!any) break;
  }
  // the floor a step may not take any body below: a tenth of the smallest
  // target (or of the smallest area the frame began with, when that is
  // less). Half was too high: twenty-six bodies born in one frame at the
  // minimum claim sit near half of it while the others sort themselves out,
  // and every step that reduced the residual was refused for it.
  const floor = .1 * Math.min(minOf(target), minOf(current.areas));
  // NEWTON WITH A LINE SEARCH, as Astra I: the Newton direction, taken
  // whole when that lowers the residual by at least half the step's share,
  // else halved, up to twelve times. The direction is kept and the length
  // is not: a damped step (the diagonal raised, the direction bent toward
  // each body's own residual) was tried in this seat and refused its way
  // to a wall on every hard frame this scenario has — twenty-six births in
  // one frame, a page of arrivals crushing the tiles beside them — where
  // halving found a step every time. The budget is in evaluations,
  // whatever it took: a frame is never held for a solve that is not
  // converging.
  let iterations = 0, maxRel = Infinity;
  const budget = maxEvals + evaluations;
  for (; iterations < maxIter && evaluations < budget; iterations++) {
    // a step that buried a group: revive it before the next
    for (let i = 0; i < n; i++) if (current.areas[i] <= 1e-3 * target[i] && evaluations < budget) repair(i);
    ...
    // a group the last step left without a cell has an empty row: give it
    // the lower-quartile diagonal of the page, so its step is its residual
    // over a typical front, a revival rather than a singular system
    const diag = Array.from({ length: n }, (_, i) => J[i][i]).sort((a, b) => a - b), mu = diag[n >> 2];
    for (let i = 0; i < n; i++) if (J[i][i] < 1e-3 * mu) J[i][i] = mu;
    const norm = norm2(residual), dw = solvePinned(J, residual);
    let accepted = false;
    for (let t = 1, k = 0; k < 12 && evaluations < budget; k++, t /= 2) {
...
// A field reads its cell EXACTLY, seam vertices included. An earlier version
// rounded the outline off at 0.05 px so a tile would read as a rectangle to
// the hive it holds; the reference's own domainFor warns why not: the field's
// leaves then cover the rounded polygon while the neighbours cover the exact
// one, and the sliver between (0.7 px² in a 240 Hz sidebar frame, at a seam
// that happened to sit on a raster sample column) reads as a 72 px² gap. At
// rest the lattice makes every seated cell an exact rectangle anyway, so the
// field locks without help.
...
Hive.prototype.solve = function() {
  ...
  if (allSeated) {
    ...
    sol = tesseraSolve(seeds, owners, offs, targets, w0, this.domainPts());
