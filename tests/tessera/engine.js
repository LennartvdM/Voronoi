 ...
 * its own clock. A footprint pushed off the page keeps its area inside by
 * weight. Whitespace is whatever the tiles are not: a void is its
...
const TES_PACK_ITERS = 6;      // packing sweeps per motion step
...
function tesseraSolve(seeds, owner, offset, targets, w0, bounds, maxIter = 16) {
...
Hive.prototype.tesFootprint = function(b) {
  ...
  if (b.rect && !b.leaving) {
    ...
    if (j && j.stamp && b.sizeFrom) {
      // a tile changes size on the way, on the same curve it travels
      const e = b.progress;
...
Hive.prototype.tesPack = function(h) {
  ...
  for (const b of this.bodies) {
    ...
    items.push({ b, hw: fp.hw * fp.mix, hh: fp.hh * fp.mix, x: b.x + p.x * pk, y: b.y + p.y * pk, comp, pk, wx: b.x, wy: b.y });
  }
  const live = new Set();
  for (let it = 0; it < TES_PACK_ITERS; it++) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let j = i + 1; j < items.length; j++) {
        ...
        if (axis === 'x') { A.x -= sx * ox * share; B.x += sx * ox * (1 - share); }
        else { A.y -= sy * oy * share; B.y += sy * oy * (1 - share); }
        moved = true;
      }
    }
    if (!moved) break;
...
function tesAxis(c, half, pitch) {
  const K = Math.max(1, Math.round(2 * half / pitch));
  if (K === 1) return [c];
...
Hive.prototype.placeSeeds = function() {
  ...
  for (const b of this.bodies) {
    ...
    const fx = b.x + p.x * pk, fy = b.y + p.y * pk;
  ...
  for (const b of this.bodies) {
    ...
    b.claim = Math.max(CLAIM_MIN, free / (PW * PH));
  }
  // 3. the sites
  const tiles = [];
  for (const b of this.bodies) {
    ...
    if (fp && mix >= 1 - 1e-9) {
      ...
      b.tesLat = { x0: fx - fp.hw, x1: fx + fp.hw, y0: fy - fp.hh, y1: fy + fp.hh, xs, ys,
        colRows: new Map([[xs[0], new Set(ys)], [xs[xs.length - 1], new Set(ys)]]),
        rowCols: new Map([[ys[0], new Set(xs)], [ys[ys.length - 1], new Set(xs)]]) };
      tiles.push(b);
    } else if (fp && mix > 0.02) {
  ...
  const carry = (T, LT, axis, at, v, A, B, lo, hi) => {
    ...
    for (const have of sets) if (Math.abs(have - v) < 1e-6) return false;
    if (occluded(A, B, lo, hi, axis, v)) return false;
    const e = axis === 'x' ? Math.max(0, LT.y0 - v, v - LT.y1) : Math.max(0, LT.x0 - v, v - LT.x1);
    if (e >= (axis === 'x' ? PH : PW) / 2) return false;
    sets.add(v);
    T.subs.push({ body: T, x: axis === 'x' ? at : v, y: axis === 'x' ? v : at, w: T.subs[0].w, off: 4 * e * e, claim: T.subs[0].claim, live: true, mirror: true });
    return true;
  };
  ...
  for (const v of tiles) {
    ...
    const LV = v.tesLat, w = v.subs[0].w, claim = v.subs[0].claim;
    for (const T of content) {
      const L = T.tesLat;
      if (L.x1 < LV.x0 - PW || L.x0 > LV.x1 + PW || L.y1 < LV.y0 - PH || L.y0 > LV.y1 + PH) continue;
      const ring = [];
      for (const r of L.ys) ring.push([L.x1 + PW / 2, r], [L.x0 - PW / 2, r]);
      for (const c of L.xs) ring.push([c, L.y1 + PH / 2], [c, L.y0 - PH / 2]);
      ring.push([L.x1 + PW / 2, L.y1 + PH / 2], [L.x0 - PW / 2, L.y1 + PH / 2], [L.x1 + PW / 2, L.y0 - PH / 2], [L.x0 - PW / 2, L.y0 - PH / 2]);
      for (const [x, y] of ring) {
        const e = Math.max(0, LV.x0 - x, x - LV.x1, LV.y0 - y, y - LV.y1);
        if (e >= PW / 2) continue;
        v.subs.push({ body: v, x, y, w, off: 4 * e * e, claim, live: true, ringOf: T });
      }
    }
    for (const s of v.subs) {
      ...
      if (v.leaving) for (const o of tiles) {
        if (!o.isVoid || o.leaving || o === v) continue;
        const L = o.tesLat;
        const out = Math.max(L.x0 - s.x, s.x - L.x1, L.y0 - s.y, s.y - L.y1);
        if (out < PW / 2) { const u = out <= 0 ? 1 : 1 - out / (PW / 2); pen = Math.max(pen, 8 * PW * PW * u * u); }
      }
      s.off += pen;
    }
  }
...
Hive.prototype.solve = function() {
  ...
  if (allSeated) {
    ...
    const indices = new Map(groups.map((b, i) => [b, i]));
    const owners = subs.map(s => indices.get(s.body)), seeds = subs.map(s => [s.x, s.y]), offs = subs.map(s => s.off);
    sol = tesseraSolve(seeds, owners, offs, groups.map(b => b.subs[0].claim), groups.map(b => b.tesW || 0), this.domainPts());
    groups.forEach((b, i) => { b.tesW = sol.groupWeights[i]; });
