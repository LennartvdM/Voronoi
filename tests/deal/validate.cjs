// Deal is Cue with the niche discarded: a cell dealt to the stage flies as
// a card, and the pen closes over the gap it leaves, the half of the pen's
// bento beside the gap stretched over it, until the card comes home.
// Without the Deal button, the real tick must produce exactly the same state
// and the same drawing commands as Cue, a click, a scroll, a Tell story, a
// Tell II story and home included; with the story, every slide settled has
// its cast on its places as rectangles and the pen whole (every cell of it on
// its rectangle, the rectangles tiling the pen, no whitespace in it), a
// sequence builds up one cell a slide with the cells already on the stage
// never moving, a fresh slide's cell is none of the last sequence's, a cell of
// the pen whose rectangle stays holds still, no two cards in flight
// overlap, the whitespace is exact and covers the page, no cell is a field,
// nothing fractures on any frame of a change, a notch turns one slide, Escape
// goes home, and the same holds on a phone.
...
const files=[path.resolve(__dirname,'../../cue.html'),path.resolve(__dirname,'../../deal.html')];
...
// --- a click, a scroll, a drag, a Tell and a Tell II story and home, without Deal: Cue's, exactly
{
 ...
 es.forEach(e=>e.home());advance(300);
 assert(!es[1].cue(),'a Deal story without the Deal button');
...
// its place, a rectangle to 4 px, unlabelled; the pen whole, every other cell
// on its rectangle of it, the rectangles tiling it, no whitespace in it;
// every cell one site, none a
...
function settled(st,k,label){
 ...
 const pen=e.cuePen(),frame=e.penFrame(),held=roots(e).filter(b=>!T.on.includes(b));
 assert.equal(pen.length,held.length,label+': the pen is laid for '+pen.length+' cells, '+held.length+' are home');
 for(const b of held)assert(pen.some(r=>rectsEqual(b.rect,r)),label+': '+b.name+' is not on its rectangle of the pen');
 assert.equal(new Set(held.map(b=>pen.findIndex(r=>rectsEqual(b.rect,r)))).size,held.length,label+': two cells on one rectangle of the pen');
 for(const b of held)assert(b.rect[0]>=frame[0]-1e-9&&b.rect[1]>=frame[1]-1e-9&&b.rect[2]<=frame[2]+1e-9&&b.rect[3]<=frame[3]+1e-9,label+': '+b.name+' is outside the pen');
 for(let i=0;i<held.length;i++)for(let j=i+1;j<held.length;j++)assert(!overlap(held[i].rect,held[j].rect),label+': two cells of the pen overlap');
 const penArea=held.reduce((q,b)=>q+(b.rect[2]-b.rect[0])*(b.rect[3]-b.rect[1]),0);
 assert(Math.abs(penArea-(frame[2]-frame[0])*(frame[3]-frame[1]))<1e-6,label+': the pen is not whole, its cells cover '+penArea.toFixed(4)+' of '+((frame[2]-frame[0])*(frame[3]-frame[1])).toFixed(4));
 assert(!e.root.bodies.some(v=>v.isVoid&&!v.leaving&&v.rect&&overlap(v.rect,frame)),label+': whitespace in the pen');
 ...
 const vx=voidsExact(st,4,[e.penFrame()]);assert(vx.ok,label+': '+vx.why);   // to 4 px: no weights are handed, and the live auction settles a hair off the authored diagram where two pieces meet a cell's corner
 ...
 return{k,cast:T.on.map(b=>b.name),pen:held.length,offRect:+worst.toFixed(1),voids:voids.length,rest:{rectangle:sh.rectangle,voronoi:sh.voronoi,cut:sh.cut}};
}
// A CHANGE: the cells on the stage that the slide keeps hold still, so does a
// cell of the pen whose rectangle stays, and one whose rectangle
// changes (it closes a gap, or gives ground back) stays in the pen, no two
...
function change(st,k,label){
 const e=st.e,T=e.cue(),from=T.k,was=T.on.slice(),before=new Map(roots(e).map(b=>[b,{x:b.x,y:b.y,rect:b.rect.slice(0,4)}])),fresh=!!e.slides2()[k].fresh&&k>from,frame=e.penFrame();
 e.cueGo(k);st.step(2);
 ...
 const penStay=roots(e).filter(b=>!now.includes(b)&&!was.includes(b)),penMoved=penStay.filter(b=>!rectsEqual(b.rect,before.get(b).rect));
 const penStill=penStay.filter(b=>!penMoved.includes(b));   // a cell of the pen on the same rectangle holds still, as the cast that stays does
 for(const b of penMoved)assert(b.rect[0]>=frame[0]-1e-9&&b.rect[1]>=frame[1]-1e-9&&b.rect[2]<=frame[2]+1e-9&&b.rect[3]<=frame[3]+1e-9,label+': '+b.name+' left the pen to close a gap');
 let frames=0,frac=0,stayMove=0,penMove=0,close=Infinity,over=0;
 ...
 while(e.changeLeft()>0&&frames<600){st.step(1);frames++;frac+=shapes(st).fractured;for(const b of kept){const p=before.get(b);stayMove=Math.max(stayMove,Math.hypot(b.x-p.x,b.y-p.y));}for(const b of penStill){const p=before.get(b);penMove=Math.max(penMove,Math.hypot(b.x-p.x,b.y-p.y));}
 ...
 assert(penMove<1,label+': a cell of the pen on the same rectangle moved '+penMove.toFixed(1)+' px');
 st.step(240);
 return{k,fresh,cast:now.map(b=>b.name),kept:kept.length,penStayed:penStay.length,penStretched:penMoved.length,closest:close===Infinity?null:+close.toFixed(2),cardOverlap:+over.toFixed(4),seconds:+(frames/60).toFixed(1)};
}
...
const result={scope:'Real full tick; native Canvas and browser DOM stubbed. Without the Deal button, state and every drawing command must exactly match Cue, a click, a scroll, a Tell story, a Tell II story and home included; with the story, every slide settled has its cast on its places (rectangles to 4 px, unlabelled) and the pen whole (every cell of it on its rectangle, the rectangles tiling it, no whitespace in it), a sequence builds up one cell a slide with the cells already on the stage never moving, a fresh slide brings out a cell that was none of the last sequence, a cell of the pen whose rectangle stays holds still and one that closes a gap stays in the pen, the whitespace is exact and covers the page, no field, one site a cell, nothing fractures on any frame of a change, no two cards in flight overlap, a notch turns one slide, and the same on a phone.',frames:total,drawingCommands:commandsTotal,matrix,report};
