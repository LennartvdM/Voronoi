...
(async()=>{
 ...
 for(const width of [1440,390]){
  ...
  s=s.slice(0,i)+'\nwindow.T={get:()=>({root,picture,W,H}),rasterCheck,ringArea,clipHalfPlane,mouse:(x,y)=>{mouseX=x;mouseY=y}};\n'+s.slice(i);
  ...
  const result=await p.evaluate(()=>{
   ...
   window.runPhase=(label,frames,hover=false)=>{
    let gap=0,over=0,vanish=0,invalid=0,err=0,areaError=0,roles=0,maxPositionJump=0,maxVelocityJump=0,maxLoanCredit=0;
    for(let k=0;k<frames;k++){
     ...
     for(const b of r.bodies){if(!Number.isFinite(b.x+b.y+b.vx+b.vy))invalid++;if(!b.isVoid&&!b.leaving){let va=0;for(const [loops,sign]of [[b.loops||[],1],[b.holes||[],-1]])for(const lp of loops){let pc={pts:lp,labs:lp.map(()=>-1)};for(const [a,c,d] of [[-1,0,0],[0,-1,0],[1,0,W],[0,1,H]]){if(pc.pts.length<3)break;pc=X.clipHalfPlane(pc,a,c,d,-1);}va+=sign*Math.abs(X.ringArea(pc.pts));}if(va<1)vanish++;if(b.rect){const key=b.rect.join(',');if(assigned.has(key))roles++;assigned.add(key);}}}
     ...
     maxLoanCredit=Math.max(maxLoanCredit,r.a3Credit||0);
     err=Math.max(err,r.solved?.maxRelErr||0);areaError=Math.max(areaError,Math.abs(r.bodies.reduce((sum,b)=>sum+(b.paintArea||0),0)-W*H-(r.a3Credit>.1?r.a3Credit:0)));
    ...
    const r=X.get().root;
    const row={label,frames,gap,over,vanish,invalid,err,areaError,roles,maxPositionJump,maxVelocityJump,maxLoanCredit,unseated:r.bodies.filter(b=>!b.isVoid&&!b.leaving&&b.rect&&!b.wall).length};
