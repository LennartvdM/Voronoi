...
const files={skim:path.join(__dirname,'skim.html'),caption:path.resolve(__dirname,'../../caption.html')};
const cases=[
...
for(const cfg of cases){
 for(let w=0;w<2;w++)for(const name of ['skim','caption'])workload(loadEngine(files[name],cfg),false,cfg.moving);
 const samples={skim:[],caption:[]};
 for(let rep=0;rep<5;rep++)for(const name of rep%2?['caption','skim']:['skim','caption'])samples[name].push(workload(loadEngine(files[name],cfg),true,cfg.moving));
 ...
 const row={...cfg,framesPerRun:1600,repeats:5,medians,reductionPct:100*(1-medians.caption.totalMs/medians.skim.totalMs),samples};
