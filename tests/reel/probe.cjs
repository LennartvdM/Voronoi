...
function loadEngine(file,{width=1900,height=810,fields=.55,count=12,profile=false,record=false,transform=x=>x}={}) {
 ...
 js=js.slice(0,end)+instrument+`\nreturn {root,config,init,resize,computeDiagram,solveWeights,rasterCheck,advance(ms){tick(ms);return picture;},pointer(x,y){mouseX=x;mouseY=y;},scene(n){config.scene=n;root.enterScene(n);},time(){return simTime;},changeLeft(){return guestLeft;},click(x,y){return portalClick(x,y);},home(){portalHome();},focus(){return portalFocus;},kinds(){return PORTAL_KINDS;},templates(){return PORTAL_TEMPLATES;},page(name,n){return portalScene(root,name,n);},flip(v){portalFlip=!!v;return portalFlip;},scroll(dy){return reelMove(dy);},wheel(dy){return reelScroll(dy);},drift(v){if(v!==undefined)reelDrift=v;return reelDrift;},flow(){return reel?reel.strips.map(S=>S.v):null;},dragStart(x,y,t){return reelDragStart(x,y,t);},dragMove(x,y,t){return reelDragMove(x,y,t);},dragEnd(t){return reelDragEnd(t);},reel(){return reel;},decorate(c,v,w){return portalDecorate(c,v,W,H,root.COLS,root.ROWS,w);},convexPlanes,graph(){return bodyGraph(root);},passage(o){return passageTimes(bodyGraph(root),root.sourcesFor(o,null),config.stagger,40+root.serial+1,root.shift);},scenes,rectsEqual,end(){return changeEnds(root);},inner(n){config.inner=n;root.walk(h=>{if(h.depth>0)h.enterScene(n);});}};\n`+js.slice(end);
 const noop=()=>{}, commands=[];
 const ctx={};
