...
slots sliding under it. The visible bands of the strip are the page's own
layout; the bands beyond it follow a pattern that keeps the cards varied.
...
replace('&larr; Back · Portal</a> <span style="opacity:.55;margin-left:.6em">click a cell to open its page · click the image again for home</span>',
        '&larr; Back · Reel</a> <span style="opacity:.55;margin-left:.6em">click a cell to open its page · scroll to browse its gallery · click the image again for home</span>')
...
function reelEnter() {
  ...
  for (const st of spec.strips) {
    ...
    offset += st.k;
    const g = st.g, S = { def: st, cards, s: 0, M: st.k, parkEdge: 1, voidU0: false, voidU1: false, pageU0: false, pageU1: false, pageT: false };
...
// REEL: the wheel browses the open page's gallery
canvas.addEventListener('wheel', (e) => { if (!reel) return; e.preventDefault(); reelScroll(e.deltaY); }, { passive: false });''')
