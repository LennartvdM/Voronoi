"""Build the Lintel mark, derived from Pane without modifying the published mark: whitespace that is the rectangle it was given.
...
replace('<title>Hive</title>', '<title>Lintel — Hive</title>')
replace('&larr; Back</a>', '&larr; Back · Lintel</a>')
...
# development only: LINTEL_VARS="PL_SWELL=0" builds a variant
for kv in filter(None, os.environ.get('LINTEL_VARS', '').split(',')):
    kk, vv = kv.split('=', 1)
    pat = re.compile(r'^(const %s = )([^;]+);' % re.escape(kk), re.M)
    n = len(pat.findall(s))
    if n != 1:
        raise SystemExit(f'LINTEL_VARS: {kk} found {n} times')
    ...
    for (const b of this.bodies) {
      ...
      if (b.isVoid && this.depth === 0 && PN_PANE) {
        ...
        if (rr) {
          ...
          // Keep the cut in the decomposition for the void's whole lifetime.
          // Switching a domain piece on at the first visible width is the
          // discontinuity Pane was meant to avoid; a sub-pixel pilot keeps
          // the topology stable while remaining visually and numerically
          // negligible. pnS still owns the actual sweep and ledger transfer.
          const cut = Math.max(PN_EPS, b.pnS);
          b.wall = [px0, py0, px0 + (px1 - px0) * cut, py1];
          this.walls.push(b);
        }
        continue;
      }
...
"""  this.plSeamSites();
  ...
  if (this.depth === 0 && PN_PANE) {
    for (const b of this.bodies) {
      ...
      if (!b.isVoid || b.pnS === undefined) continue;
      ...
      if (sub) {
        const cut = Math.max(PN_EPS, b.pnS);
        sub.claim = Math.max(CLAIM_MIN, b.claim - cut * a);
        const x0 = r[0] + (r[2] - r[0]) * cut;
...
OUT = Path(os.environ.get('LINTEL_OUT', str(ROOT / 'lintel.html')))
OUT.write_text(s)
if os.environ.get('LINTEL_OUT'):
...
card = '''
        <!-- LINTEL -->
        <a href="lintel.html" class="version-card">
            <h2>Lintel</h2>
            <span class="mk">Lintel</span>
            <span class="tag experimental">Rejected experiment</span>
            <p>Pane&rsquo;s straight whitespace with a persistent, sub-pixel pilot cut. The pilot keeps the ground&rsquo;s convex decomposition present for the void&rsquo;s whole lifetime, rather than switching it on when the visible band begins to grow.</p>
            <ul>
                <li>Straight void targets retained from Pane</li>
                <li>Stable decomposition topology across the pane sweep</li>
                <li>Void claims stay balanced against the pilot cut</li>
                <li>Published as a new mark so its animation can be evaluated live beside Pane and Plumb</li>
            </ul>
        </a>
        <!-- /LINTEL -->
'''
...
IS_LATEST = False  # retained as a visible failed branch, never promoted
if IS_LATEST:
    index = index.replace(' class="version-card latest"', ' class="version-card"').replace('<span class="latest-flag">Latest</span>', '')
if not IS_LATEST:
...
print('Built lintel.html from reference blob', EXPECTED)
print('Lintel SHA256', hashlib.sha256(s.encode()).hexdigest())
